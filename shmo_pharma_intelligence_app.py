#!/usr/bin/env python3
"""
ShMo Pharma Intelligence - Consolidated Single-File Application
Flask dashboard for Pharma Competitive Intelligence (port 5001)
All modules integrated: regulatory_sources, biohealth_news, pipeline_impact, article_extractor, summarizer
"""
...
def _safe_get(url: str, **kwargs) -> Optional[requests.Response]:
    try:
        return requests.get(url, headers=_BH_HEADERS, timeout=20, **kwargs)
    except requests.RequestException as exc:
...
_DEMO_CLUSTERS = {
    0: [
        {"title": "Novo Nordisk Wins FDA Nod for Ozempic in CKD", "link": "https://example.com/novo-fda",
         ...
         "summary": {"bullet_points": ["Semaglutide received FDA approval for CKD in type 2 diabetes patients.",
                                      "Phase 3 FLOW trial showed 24% reduction in kidney disease progression."],
                     "executive_summary": "Novo Nordisk secured FDA approval for semaglutide in CKD.",
                     "drug_name": "semaglutide (Ozempic)", "company": "Novo Nordisk",
                     "indication": "CKD in type 2 diabetes", "regulatory_status": "FDA approved"}},
        {"title": "Eli Lilly Launches Phase 3 Renal Trial for Mounjaro", "link": "https://example.com/lilly-renal",
         ...
         "summary": {"bullet_points": ["Tirzepatide Phase 3 renal outcomes trial initiated."],
                     "executive_summary": "Eli Lilly launched Phase 3 trial for tirzepatide renal outcomes.",
                     "drug_name": "tirzepatide (Mounjaro)", "company": "Eli Lilly",
                     "indication": "CKD in type 2 diabetes", "regulatory_status": "Phase 3 initiated"}},
    ],
    1: [
        {"title": "Bayer Finerenone 4-Year Data Reinforces CKD Benefits", "link": "https://example.com/bayer",
         "source": "Endpoints News", "published_date": "Feb 18, 2026", "keyword": "CKD treatment",
         "summary": {"bullet_points": ["Bayer's finerenone showed long-term renal benefits."],
                     "executive_summary": "Bayer presented 4-year data confirming finerenone benefits.",
                     "drug_name": "finerenone (Kerendia)", "company": "Bayer",
                     "indication": "CKD in type 2 diabetes", "regulatory_status": "Approved"}},
    ],
    -1: [
        {"title": "Global CKD Prevalence Rising in Asia-Pacific", "link": "https://example.com/ckd-apac",
...
@app.route("/")
def index():
    # Serve external HTML if it exists (same dir as script), else embedded
    html_path = Path(__file__).resolve().parent / "shmo_pharma_intelligence_dashboard.html"
    if html_path.exists():
